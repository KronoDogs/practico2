...
## Modelos de Regresión Lineal Simple y Múltiples

En esta parte de nuestra investigación, propondemos distintos modelos con variables que no se han utilizado hasta el momento. En los siguientes modelos de regresión comprenderemos la variable dependiente "indicexen1" la cual contiene todas las variables anteriormente utilizadas, que miden opinión de los encuestados sobre diferentes ámbitos de la población migrante. Los siguientes modelos contienen 3 variables independientes en respecto de la dependiente, de forma lineal en primera instancia para cada modelo, por tanto, se establecerá la asociación entre este indice según clase social, la confianza en los medios y la edad de los encuestados.
...
Como se puede observar, la variable confianza en los medios contiene un intercepto de 1.78, lo cual indica que cuando la confianza en los medios de comunicación es 0, en promedio, las personas tenderán a tener un puntaje de 1.78, indicando un nivel medio de la variable dependiente. En este mismo sentido, el segundo modelo indica que las personas con 0 años tienen un nivel de xenofobia de 1.81, lo cual a nivel sustantivo, no proporciona ninguna información. Y por último, el intercepto del tercer modelo menciona que cuando la clase social de las personas es 1, su nivel de xenofobia será de 1.78 puntos.

Con respecto a los coeficientes de regresión de cada modelo, podemos evidenciar que en las confianza en los medios es de -0.01, lo cual indica que a medida que aumenta en una unidad la confianza en los medios de comunicación, el indice de xenofobia de los encuestados disminuirá 0.01 en respecto a la escala de medición de la variable dependiente. Sin embargo, este coeficiente no es significativo estadísticamente, ya que el valor p es mayor a 0.05. Por tanto, no habría una relación significativa entre la variable dependiente y la independiente, lo cual, también se puede evidenciar por su respectivo coeficiente de regresión. Del mismo modo ocurre en en el modelo 2, en donde su coeficiente de regresión es de -0.00, este valor prácticamente indica que no hay asociación entre ambas variables, ya que su valor p no es significativo, ya que es mayor a 0.05. Y en respecto del modelo 3 ocurre lo mismo, ya que, observamos el coeficiente de regresión del modelo es -0.01, indicando que cuando sube en una unidad la clase social de las personas, el indice de xenofobia bajará en 0.01. Y lo mismo ocurre con los anteriores modelos, este coeficiente no es significativo, por tanto, no hay una relación significativa entre la clase social y la variable dependiente. Esto último se respalda con el R cuadrado, ya que como se observa que en todos los modelos tienen este valor de 0.00, lo que significa que las variables independientes incluidas en los modelos no explican una cantidad significativa de variabilidad de la variable dependiente, de este modo, los modelos no son estadísticamente relevantes y no son adecuados para predecir la variable del índice de xenofobia.

## Modelos de Regresión Múltiple

En este apartado propondremos modelos de regresión múltiple utilizando las variables de los modelos de regresión simple.
...
En el modelo 1 propone como variable dependiente el Índice de Xenofobia propuesto en el inicio del apartado de este tipo de modelos, y las variables que buscan explicar el comportamiento de esta variable la confianza en los medios de comunicación, la edad y la clase social. En el caso del intercepto, cuando todas las variables independietnes son iguales a cero, el índice de xenofobia es de 1.79 puntos, en una escala de 0 a 4. En donde el valor es altamente significativo, debido que, el valor p menor a 0.001. En el caso de la confianza en los medios, se interpreta que hay una disminución en su asociación de un 0.01 en respecto al índice de xenofobia. Sin embargo, los resultados obtenidos no son estadísticamente significativos. En el caso de la edad no tiene ningún impacto signficativo, ya que su coeficiente de regresión es 0.00, por tanto, no habría una asociación entre ambas variables. Y con la clase social podemos evidenciar tiene un reducido efecto, aunque no llega a ser significativo, ya que se observa que por cada aumento unitario en las categorías de la clase social, el aumento del índice de la xenofobia presentará un incremento de 0.01, en donde el coeficiente de regresión es muy cercano a 0. Y en línea general, el modelo presenta un R cuadrado de un 0% de la variabilidad en el índice de xenofobia, lo que sugiere que las variables independientes incluidas en el modelo no lograr capturar la varianza de esta variable de interés, por tanto, no resultan como buenos predictores.

En el caso del modelo 2 podemos evidenciar que su intercepto es de 1.77, con un valor altamente significativo, ya que el valor p es menor a 0.001. En respecto a la confianza en los medios y la clase social no tienen un efecto significativo en la variable dependiente, que es el mismo índice de xenofobia. En el caso de la variable de los niveles educativos desglosados en sus diferentes categorías, encontramos que la Educación Universitaria incompleta presenta el menor coeficiente de regresión, indicando que las personas con este nivel educativo tienen un índice de xenofobia de un 0.03 mayor que la categoría de referencia, que es el nivel educativo de Sin estudios. De la misma forma, el Nivel Medio de educación tiene un coeficiente de regresión de un 0.05 puntos mayor que la categoría de referencia, y le sigue el nivel Universitario completo, el cual tiene un coeficiente de 0.08 en su índice de xenofobia. Y en el caso del nivel educacional de los institutos, los incompletos tienen un coeficiente de regresión de 0.11 puntos de índice de xenofobia y por último los completos, presentan un coeficiente de 0.16 puntos de índice de xenofobia. En conclusión, este modelo no resulta adecuado para predecir la variable dependiente, lo que se corresponde con los valores obtenidos en cada coeficiente de regresión de las variables propuestas, ya que cada valor es cercano a 0, por tanto sus respectivos Betas no presentan una asociación estadísticamente significativa, por tanto, estas variables no son adecuadas para predecir valores del índice de la xenofobia.
...
## Cálculo de valores predichos de Modelos de Regresión
